...
#define TEST_CASE(buf,e,s,idx,subidx,lg,exp,comp)              \
    ret=CO_SDO_build_init_ul_rp(buf,e,s,idx,subidx,lg); \
    if(ret!=exp) {                                      \
        fprintf(stderr,                                 \
            "%d: CO_SDO_build_init_ul_rp failed, error(%d), expected(%d)\n",    \
            __LINE__, ret, exp);                        \
        return 1;                                       \
    }                                                   \
    else {                                              \
...
static const unsigned char compare_buf1[] = { 0x41, 0x84, 0x01, 0xa5, 0x34, 0x12, 0x00, 0x00 } ;
static const unsigned char compare_buf2[] = { 0x41, 0x84, 0x01, 0xa5, 0x12, 0x00, 0x00, 0x00 } ;
static const unsigned char compare_buf3[] = { 0x41, 0x84, 0x01, 0xa5, 0x45, 0x23, 0x01, 0x00 } ;
static const unsigned char compare_buf4[] = { 0x41, 0x84, 0x01, 0xa5, 0x56, 0x34, 0x12, 0x00 } ;
static const unsigned char compare_buf5[] = { 0x41, 0x84, 0x01, 0xa5, 0x67, 0x45, 0x23, 0x01 } ;
static const unsigned char compare_buf6[] = { 0x4F, 0x84, 0x01, 0xa5, 0x01, 0x00, 0x00, 0x00 } ;  
...
int main(void) {
    ...
    TEST_CASE(can_buff, 1, 1, idx, subidx, 0x1, CO_ERROR_NONE, compare_buf6)
