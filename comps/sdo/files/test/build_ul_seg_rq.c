...
#define TEST_CASE(buf,toggle,exp,comp)              \
    ret=CO_SDO_build_ul_seg_rq(buf,toggle); \
    if(ret!=exp) {                                      \
        fprintf(stderr,                                 \
            "%d: CO_SDO_build_ul_seg_rq failed, error(%d), expected(%d)\n",    \
            __LINE__, ret, exp);                        \
        return 1;                                       \
    }                                                   \
    else {                                              \
...
static const unsigned char compare_buf1[] = { 0x60, 0, 0, 0, 0, 0, 0, 0 } ;
static const unsigned char compare_buf2[] = { 0x70, 0, 0, 0, 0, 0, 0, 0 } ;
